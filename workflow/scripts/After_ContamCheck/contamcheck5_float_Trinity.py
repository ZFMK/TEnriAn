...
Blastresult="blast_all_vs_all" # name of the output file from blast all versus all (-evalue 1e-50, -outfmt 6)

if path.exists("blast_all_vs_all"):
    print("blastresult provided, start filtering")
else:
    subprocess.run("makeblastdb -in "+Sequences+" -out mydb -dbtype nucl",shell=True)
    subprocess.run("blastn -query "+Sequences+" -db mydb -evalue 1e-50 -out "+Blastresult+" -outfmt 6 -perc_identity 99 -num_threads 8",shell=True)

Column = 10   # the readnumber is x-th element of the fasta header, when seperated by underscores (~ abundance)
...
FILTERED = open("filtered_blast_result.txt","w")
...
FILTERED = open("filtered_blast_result.txt","r")
BLACKLIST = open("blacklist.txt","w")
PAIRS = open("blast_pairs.txt","w")
...
subprocess.run("sort blacklist.txt | uniq > blacklist_uniq.txt", shell=True)
subprocess.run("cut -f1 -d'_' blacklist_uniq.txt | uniq -c > how_many_contam_per_species.txt", shell=True)
subprocess.run("sort blast_pairs.txt | uniq -c | sort -nr > species_pairs_by_occurrence.txt", shell=True)
subprocess.run("rm blacklist.txt blast_pairs.txt", shell=True)

# makes the white list (contamination free sequences)

subprocess.run("grep '>' "+Sequences+" | sed 's/>//' > all_headers.txt", shell=True)
subprocess.run("cat blacklist_uniq.txt all_headers.txt | sort | uniq -u > whitelist.txt", shell=True)
subprocess.run("rm all_headers.txt",shell=True)
...
subprocess.run("python seqout blacklist_uniq.txt "+Sequences+" seq_contaminated.fasta", shell=True)
subprocess.run("python seqout whitelist.txt "+Sequences+" seq_contam_free.fasta", shell=True)

#sorts sequences into single species files

subprocess.run("mkdir free_by_species", shell=True)
Oldname = "   "
flag = 1
FILE = open("seq_contam_free.fasta","r")
for Line in FILE:
    Line = Line.strip()
    if ">" in Line:
        Items = Line.split("_")
        ...
        if Name == Oldname:
            ...
            if flag == 1:
                NEWSPEC=open("free_by_species/"+Name+".fas","w")
                NEWSPEC.write(Line+"\n")
                Oldname=Name
                flag = 2
            else:
...
subprocess.run("mkdir contam_by_species", shell=True)

Oldname = "   "
flag = 1
FILE = open("seq_contaminated.fasta","r")
for Line in FILE:
    Line=Line.rstrip()
    if ">" in Line:
        Items = Line.split("_")
        ...
        if Name == Oldname:
            ...
            if flag == 1:
                NEWSPEC=open("contam_by_species/"+Name+".fas","w")
                NEWSPEC.write(Line+"\n")
                Oldname=Name
                flag = 2
            else:
